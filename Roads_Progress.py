...
def push_to_github(file_path, commit_message):
    """Pushes the local CSV file back to the GitHub Repository."""
    if not GITHUB_TOKEN or not REPO_NAME:
        st.error("GitHub Secrets not configured. Changes saved locally only.")
        return
...
def load_shapefile():
    ...
    # Apply manual assignment overrides if they exist
...
def save_completed(editor, suburbs_selected):
    ...
    # Auto-sync to GitHub
    push_to_github(SAVE_FILE, f"Update completion for {editor}")
...
with st.sidebar.expander("⚙️ Admin: Reassign Suburbs"):
    st.write("Change who is responsible for a suburb.")
    ...
    st.caption(f"Current owner: {current_owner}")
    
    new_owner = st.selectbox("Reassign to:", editors)
    
    if st.button("Confirm Reassignment"):
        # Load or create overrides
        if os.path.exists(ASSIGNMENT_FILE):
        ...
        st.success(f"Moved {sub_to_change} to {new_owner}!")
        st.cache_data.clear() # Clear cache to force reload of shapefile
        st.rerun()
...
selected_suburbs = st.sidebar.multiselect(
    f"✅ Mark completed for {selected_editor}",
...
if st.sidebar.button("Save Progress"):
    ...
    st.sidebar.success("Progress Saved & Synced!")
    st.rerun()
...
# --- Legend Logic ---
legend_items = "".join([f"<i style='background:{c};width:12px;height:12px;display:inline-block;'></i> {e}<br>" for e, c in editor_colors.items()])
legend_html = f"""
{% macro html(this, kwargs) %}
<div style="position: fixed; bottom: 50px; left: 50px; width: 150px; background: white; border:2px solid grey; z-index:9999; padding: 10px;">
<b>Legend</b><br>{legend_items}
...
total_s = len(gdf)
done_s = len(gdf[gdf["status"] == "Complete"])

col1.metric("Total Progress", f"{round((done_s/total_s)*100, 1)}%", f"{done_s}/{total_s} Suburbs")
col2.progress(done_s/total_s)

summary = []
for ed in editors:
    ed_gdf = gdf[gdf["Assigned"] == ed]
    comp = len(ed_gdf[ed_gdf["status"] == "Complete"])
    summary.append({"Editor": ed, "Completed": comp, "Total": len(ed_gdf)})
